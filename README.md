...
---
![Gameplay GIF](https://via.placeholder.com/800x450.png?text=CHÈN+ẢNH+GIF+HOẶC+VIDEO+GAMEPLAY+VÀO+ĐÂY)
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Tuần 1** | **Nền móng (Foundation)** | **(Lead) Xây dựng Khung sườn (Skeleton):** <br> • Cài đặt `GameLoop`, `GameManager`. <br> • Tích hợp `GameNavigator`. | **(Lead) Thiết kế Hệ thống:** <br> • Thiết kế UML. <br> • Cài đặt sơ bộ `Brick` và `StageLoader` (tải màn). | **Lập trình Paddle:** <br> • Cài đặt sơ bộ class `Paddle` (vị trí, kích thước, hình ảnh). | **Lập trình Ball:** <br> • Cài đặt sơ bộ class `Ball` (vị trí, kích thước). |
| **(Tuần 5)** | *Tạo các class chính, tải màn chơi, vẽ các đối tượng cơ bản.* | `feat: Code Skeleton` <br> `feat: paddle brick` | `feat: brick stage` <br> (Thiết kế UML) | `Feat: introducing paddle` | `Add Ball.java` |
| | | | | | |
| **Tuần 2** | **Gameplay Cốt lõi (Core Loop)** | **(Lead) UI & Âm thanh:** <br> • `StartScreen`, `BackgroundManager`. <br> • `SoundEffectManager` (nhạc nền, SFX). | **Logic Gạch Nâng cao:** <br> • Hoàn thiện các loại Gạch (`Silver`, `Gold`). <br> • Cài đặt cơ chế *rơi* Power-up từ gạch. | **Logic Power-up (Phần 1):** <br> • Cài đặt va chạm `PowerUp` với `Paddle`. <br> • Sửa lỗi hitbox `Paddle`. | **(Lead) Logic Ball & Va chạm:** <br> • Cài đặt logic di chuyển (`velocity`). <br> • Xử lý va chạm (AABB) giữa `Ball` và `Paddle`/`Brick`. |
| **(Tuần 6)** | *Bóng nảy, gạch vỡ, có thể chơi được. Có âm thanh và UI cơ bản.* | `Feat:Sound efect, startScreen...` <br> `feat: add gameUI. fix: font` | `Power Up from brick` | `Feat: Powerups' collision...` <br> `Fix: paddle hitbox bug` | `Refactor: manageDirectionByVelocity` <br> `BallUpdate` |
| | | | | | |
| **Tuần 3** | **Hoàn thiện Tính năng (Features)** | **(Lead) Hoàn thiện UI:** <br> • `PauseScreen`, `GameOverScreen`. <br> • `GameUI` (HUD) hoàn chỉnh (điểm, mạng). | **Hệ thống Power-up (Phần 2):** <br> • Cập nhật logic HP gạch (`Brick HP update`). <br> • Thêm animation rơi cho `PowerUp`. <br> • Thêm ảnh Power-up vào `ResourceManager`. | **(Lead) Logic Power-up (Phần 2):** <br> • Cài đặt 3 Power-up chính: **`ADD_LIFE`**, **`ADD_BALL`**, **`PADDLE_GROW`**. | **Logic Game Nâng cao:** <br> • Cập nhật logic quản lý Điểm/Mạng (từ `GameManager`). <br> • Cập nhật logic `addHP` và Game Over. |
| **(Tuần 7)** | *Game có đầy đủ Power-up, các menu chức năng hoạt động.* | `Feat: update Pause, Game Over...` <br> `fix: GameUI HP and score UI` | `feat(powerup): Add falling animation...` <br> `feat(resources): add image powerup` | `Feat: Introducing Powerups: ADD_BALL, ADD_LIFE...` | `updateGameOver` <br> `addHP` |
| | | | | | |
| **Tuần 4** | **Tính năng Nâng cao & Hoàn thiện (Polish)** | **(Lead) Hoàn thiện Hệ thống:** <br> • `Scoreboard` (lưu 5 điểm). <br> • `StageClearScreen`, `HowToPlayScreen`. <br> • Tích hợp & Merge code toàn nhóm. | **(Lead) Boss & Hiệu ứng:** <br> • Cài đặt **`Meteor`** (hazard). <br> • Cài đặt **`BossBrick`** (thả Meteor, thả Power-up). <br> • Cài đặt **`Screen Shake`**. | **Tính năng Nâng cao & Kiểm thử:** <br> • Cài đặt Power-up **`Laser`**. <br> • Viết **JUnit Tests** cho dự án. | **Logic Cốt lõi Nâng cao:** <br> • Nâng cấp logic va chạm Ball/Paddle (bóng dính `isSticky`). |
| **(Tuần 8)** | *Thêm các tính năng "xịn" (Boss, Laser, Hiệu ứng) và kiểm thử.* | `Feat(ScoreBoardScreen, ScoreManager)...` <br> `Feat(StageClearScreen)...` | `feat: Implement Meteor hazard` <br> `feat: Implement screen shake...` <br> `feat(boss): Add periodic power-up...` | `Feat: New powerup - Laser` <br> `Feat: Added JUnit Tests` | `feat: Cap nhat logic va cham cho Ball va Paddle` |
